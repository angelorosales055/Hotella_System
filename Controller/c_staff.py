...
class StaffController:
    ...
    def generate_receipt(self, data, paid_now, method, payment_id):
        """Generate a styled A5 PDF receipt using ReportLab and save to receipts/<date>/."""
        if not payment_id:
            return
        ...
        try:
            ...
            hline(y + 2*mm, color=_GOLD, width=1)
            y -= 4 * mm
            ...
            hline(y + 3*mm, color=_GOLD, width=1)
            y -= 3 * mm
    ...
    def add_service_charge(self, bid, room_number, service_name, price, quantity, employee_name):
        ...
        if self.model.add_service(bid, room_number, service_name, total, date, emp_id, quantity):
            ...
            try:
                guest_details = self.model.get_active_guest(room_number)
                guest_name = guest_details[1] if guest_details else "Guest"
                self.model.add_booking_log(bid, guest_name, f"Added Service: {service_name} (x{quantity})",
                                           self.current_staff)
            except Exception as e:
        ...
        if bid:
            ...
            if pay_id and paid > 0:
                receipt_data = {
                    ...
                    'room_cost': total, 'svc_cost': 0, 'svc_details': [], 'final_total': total,
    ...
    def assign_cleaner(self, room_num, emp_name):
        ...
        if emp_id and self.model.assign_cleaner_to_room(room_num, emp_id):
            try:
                self.model.add_booking_log(0, f"Room {room_num}", f"Assigned Cleaner: {emp_name}", self.current_staff)
            except Exception:
                pass
    ...
    def finish_cleaning(self, room_num):
        if self.model.finish_cleaning_room(room_num):
            try:
                self.model.add_booking_log(0, f"Room {room_num}", "Marked Room as Clean", self.current_staff)
            except Exception:
                pass
            return True, "Room Clean"
        return False, "Failed"
