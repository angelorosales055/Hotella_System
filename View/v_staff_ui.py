...
from PyQt6.QtGui import QPalette, QColor, QPixmap, QIcon, QPainter, QPen, QPainterPath
...
class BookingPage(QWidget):
    def __init__(self, ctrl):
        ...
        self.lbl_room_details.setStyleSheet("""
            font-size: 13px; color: #333; border: 1px solid #BDC3C7; 
            padding: 12px; border-radius: 5px; background: #F9F9F9;
        """)
    ...
    def load_rooms(self):
        ...
        if self.grid.count() == 0:
            self.grid.addWidget(
                QLabel(f"No available rooms can accommodate {guests_count} guest(s) for the selected dates."))
    ...
    def select_room(self, rid, rtype, price, limit, btn):
        ...
        """
        self.lbl_room_details.setText(details_html)
    ...
    def pay(self):
        ...
        if dlg.exec() == 1:
            ...
            if res:
                ...
                receipt_dlg = ReceiptPreviewDialog(self, receipt_data, paid, method, pay_id, self.ctrl)
                receipt_dlg.exec()
...
class MaskedCardInput(QLineEdit):
    def __init__(self, *args, **kwargs):
        ...
        # 🟢 Generate Standard Vector Icons Programmatically
    ...
    def _create_eye_icon(self, hidden=False):
        """Draws a standard vector eye icon natively using QImage for cross-platform stability"""
        from PyQt6.QtGui import QImage, QPainter, QPen, QColor, QPainterPath, QIcon, QPixmap
        from PyQt6.QtCore import Qt

        # 🟢 FIX: Use QImage instead of QPixmap to guarantee the icon renders on all OS
        ...
        if raw.isdigit() or raw == '':
            ...
            if len(self.actual_value) > 0:
                if self.is_showing:
                    ...
                    formatted = self._format_card_masked(self.actual_value)
...
class CheckoutDialog(QDialog):
    def __init__(self, parent, data, ctrl):
        ...
        if data.get('svc_details'):
            l.addWidget(QLabel("Services Availed:",
                               styleSheet="font-size: 16px; font-weight: bold; color: #2980B9; margin-top: 5px;"))
            for s in data['svc_details']:
                l.addWidget(QLabel(f"  • {s['name']} (x{s['qty']}): ₱{s['total']:,}",
                                   styleSheet="font-size: 15px; color: #444;"))
        ...
        l.addWidget(QLabel(f"Already Paid: -₱{data['paid']:,}",
                           styleSheet="font-size: 18px; color: #27AE60; font-weight: bold;"))
        l.addWidget(QLabel("---------------------------------"))

        l.addWidget(QLabel(f"BALANCE DUE: ₱{self.balance_due:,}",
                           styleSheet="font-size: 26px; font-weight: 900; color: #C0392B;"))
        l.addWidget(QLabel("---------------------------------"))

        if self.balance_due > 0:
            l.addWidget(QLabel("Amount Tendered (Cash/Card):", styleSheet="font-size: 16px; font-weight:bold;"))
            self.spin = QSpinBox()
            self.spin.setRange(0, 1000000)
            self.spin.setValue(self.balance_due)
            ...
            self.cb_method.setStyleSheet(
                "padding: 10px; font-size: 16px; border: 1px solid #BDC3C7; border-radius: 5px;")
        ...
        btn.setStyleSheet(
            "QPushButton { background-color: #2C3E50; color: white; font-weight: bold; border-radius: 5px; padding: 18px; font-size: 18px; } QPushButton:hover { background-color: #34495E; }")
    ...
    def process(self):
        try:
            ...
            if res:
                receipt_dlg = ReceiptPreviewDialog(self, receipt_data, tendered, method, pay_id, self.ctrl)
                receipt_dlg.exec()
                self.accept()
            else:
...
class ReceiptPreviewDialog(QDialog):
    def __init__(self, parent, receipt_data, paid, method, pay_id, ctrl):
        super().__init__(parent)
        self.setWindowTitle("Transaction Confirmed")
        ...
        l.addWidget(
            QLabel("✅ Transaction Successful!", styleSheet="font-size: 18px; font-weight: 800; color: #27AE60;"),
        ...
        if receipt_data:
            ...
            btn_pdf.setStyleSheet(
                "background: #E67E22; color: white; padding: 12px; font-weight: bold; border-radius: 5px;")
        ...
        btn_close.setStyleSheet(
            "background: #7F8C8D; color: white; padding: 12px; font-weight: bold; border-radius: 5px;")
    ...
    def generate_pdf(self):
        if self.receipt_data:
            ...
            QMessageBox.information(self, "Success",
                                    "PDF Receipt generated successfully! Check your 'receipts' folder.")
        self.accept()